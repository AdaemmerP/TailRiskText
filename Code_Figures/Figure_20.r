...
# Date sequence for nowcasts
date_seq <- seq(as.Date("1999-08-01"), as.Date("2021-12-01"), by = "month")

# Show ggplot
df_plots_time |>
  filter(h == 1 & q == 10 & ylag == 12) |>
  ...
  mutate(
    `FRED & Topics` = `FRED & Topics` / `FRED only`,
    `FRED & Topics & SentTopics` = `FRED & Topics & SentTopics` / `FRED only`,
    `FRED only` = `FRED only` / `FRED only`
  ) |>
  pivot_longer(
    cols = c(`FRED only`, `FRED & Topics`, `FRED & Topics & SentTopics`),
    names_to = "Predictors",
    values_to = "QLmean"
  ) |>
  mutate(
    Predictors = fct_relevel(Predictors, c("FRED only", "FRED & Topics", "FRED & Topics & SentTopics"))
  ) |>
  ...
  scale_color_manual(values = c(r_colors[5], r_colors[3])) +
  labs(
    x = "Year",
