 ...
 mean_vi= meanGIG2(.5,a,b,1);
  ...
  if count>1
   ...
   betasigma=( X'*invsigma*diag(mean_inv_vi)*X/tau+ invB0)\eye(K);
    ...
    bb1 = (K-first)/2 +0;
    bb2 = 0.5*sum(betamean(1+first:end).^2+diag(betasigma(1+first:end,1+first:end))) +0;
    ridge= bb1/bb2;
  invridge=1./ridge+Input.epsilon;
  ridge=1./invridge;
  invB0(1+first:end,1+first:end)= eye(K-first)*((ridge));
