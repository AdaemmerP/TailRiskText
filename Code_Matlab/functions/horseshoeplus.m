function Output= horseshoeplus(Input)
 ...
 mean_vi= meanGIG2(.5,a,b,1);
  ...
  if count>1
   ...
   betasigma=( X'*invsigma*diag(mean_inv_vi)*X/tau+ invB0)\eye(K);
    ...
    for j = 1:K-first
    v(j)=1/(psi(j)+lambda(j));
    lambda(j)=1/(v(j)+tau_beta*(betamean(j+first)^2+betasigma(j+first,j+first))*0.5);
    psi(j)=1/(Gamma(j)+v(j));
    Gamma(j)=1/(B^2+psi(j));   
    end
  xi=1/(A^2+tau_beta);  
  tau_beta=(K-first+1)/(2*xi+sum((betamean(1+first:end).^2+diag(betasigma(1+first:end,1+first:end))).*lambda));
