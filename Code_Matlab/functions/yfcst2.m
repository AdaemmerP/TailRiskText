function [yf] = yfcst2(x,tcode,nph,    yearly)
...
elseif (tcode == 2) || (tcode == 5)
    ...
    yf(1:size(y,1)-nph) = y(1+nph:size(y,1)) - y(nph:size(y,1)-1);
    end
elseif (tcode == 3) || (tcode == 6)
        if yearly==1
    yf(2:size(y,1)-nph) = (y(2+nph:size(y,1)) - y(2:size(y,1)-nph)) -(y(2:size(y,1)-nph)-y(1:size(y,1)-nph-1));
        else
    yf(2:size(y,1)-nph) = (y(2+nph:size(y,1)) - y(nph+1:size(y,1)-1)) -(y(1+nph:size(y,1)-1)-y(nph:size(y,1)-2));
        end
elseif (tcode == 16) || (tcode == 17)
