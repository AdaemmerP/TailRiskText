function Output=adaptiveLasso(Input)
...
%meanGIG2(-.5,b,a,-1)
  ...
  A=(betamean(1+first:K).^2+diag(betasigma(1+first:K,1+first:K)));
  B=lambdamean;
  tauinv=meanGIG2(-.5,A,B,1);
  taulam=meanGIG2(-.5,A,B,-1);

  lambdamean=(lassoprior+1)./(0.5*(taulam)+lassoprior);
  invtauinv=1./tauinv;
  invtauinv=invtauinv+Input.epsilon;
  tauinv=1./invtauinv;

  invB0(1+first:end,1+first:end)= diag(tauinv);
